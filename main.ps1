...
$installDir = Read-Host -Prompt "What directory should MultiMC be installed to? (Default is user's Desktop)"

if ([string]::IsNullOrWhiteSpace($installDir)) {
    $installDir = "C:\Users\$user\Desktop"
}
...
if ($startMenuShortcut -eq 0) {
    $WScriptObj = New-Object -ComObject ("WScript.Shell")
    $SourceFilePath = "$installDir\MultiMC\MultiMC.exe"
    $ShortcutPath = "C:\Users\$user\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\MultiMC.lnk"
    $shortcut = $WscriptObj.CreateShortcut($ShortcutPath)
    $shortcut.TargetPath = $SourceFilePath
    $shortcut.Save()
} else {
